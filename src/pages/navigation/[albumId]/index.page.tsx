...
// hooks
import { useGetAlbumInfoMutation, useGetAlbumsQuery } from "hooks/navigation";
...
// types
import { AlbumListResponse, UpdateNavigationData } from "types/navigation";

function AlbumsPage(initialData: AlbumListResponse | null) {
  ...
  const { albums, totalPageCount } = useGetAlbumsQuery(
    page + 1,
    contentLimit,
    debouncedValue,
    initialData
  ...
  const { albumInfo, getAlbumInfo } = useGetAlbumInfoMutation();
  ...
  const handleOpenTracks = () => {
    push(`/navigation/23/${albumInfo?.album_id}`);
    handleDrawerClose();
  };
  ...
  const handleCellClick = (albumId: string | number) => {
    setDrawerOpen(true);
    getAlbumInfo(albumId as number);
  };
...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  try {
    const initialData = await contentService.getAlbums(
      DEFAULT_PAGE,
      DEFAULT_LIMIT,
      undefined,
      token
